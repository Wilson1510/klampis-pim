from sqlalchemy import String, event, Text, Integer, select, and_
...
class TestProductImageRelationship:
    """Test suite for Product model relationships with Image model"""
    ...
    @pytest.mark.asyncio
    async def test_create_product_with_images(self, db_session: AsyncSession):
        """Test creating product with images (valid scenario)"""
        product = Products(
        ...
        await db_session.refresh(retrieved_product, ['images'])

        assert retrieved_product.id == 3
        assert retrieved_product.name == "Test Product with Images"
        assert retrieved_product.description == "Test Description with Images"
        assert len(retrieved_product.images) == 2

        assert retrieved_product.images[0].file == "test image 1"
        assert retrieved_product.images[0].is_primary is False
        assert retrieved_product.images[0].title is None
        assert retrieved_product.images[0].object_id == retrieved_product.id
        assert retrieved_product.images[0].content_type == "products"
        parent = await retrieved_product.images[0].get_parent(db_session)
        assert parent == product

        assert retrieved_product.images[1].file == "test image 2"
        assert retrieved_product.images[1].is_primary is False
        assert retrieved_product.images[1].title is None
        assert retrieved_product.images[1].object_id == retrieved_product.id
        assert retrieved_product.images[1].content_type == "products"
        parent = await retrieved_product.images[1].get_parent(db_session)
        assert parent == product

    @pytest.mark.asyncio
    async def test_add_multiple_images_to_product(self, db_session: AsyncSession):
        """Test adding multiple images to product"""
        for i in range(5):
            image = Images(
                file=f"test image {i}",
                object_id=self.test_product1.id,
        ...
        assert len(retrieved_product.images) == 5
        for i in range(5):
            assert retrieved_product.images[i].id == i + 1
            assert retrieved_product.images[i].file == f"test image {i}"
            assert retrieved_product.images[i].is_primary is False
            assert retrieved_product.images[i].title is None
            assert retrieved_product.images[i].object_id == retrieved_product.id
            assert retrieved_product.images[i].content_type == "products"
            parent = await retrieved_product.images[i].get_parent(db_session)
            assert parent == retrieved_product

    @pytest.mark.asyncio
    async def test_update_products_images(self, db_session: AsyncSession):
        """Test updating a product's images"""
        product = await get_object_by_id(
            db_session,
            Products,
            self.test_product1.id
        ...
        product.images = [Images(file="test image 1"), Images(file="test image 2")]
        await save_object(db_session, product)
        await db_session.refresh(product, ['images'])
        assert len(product.images) == 2

        assert product.images[0].file == "test image 1"
        parent = await product.images[0].get_parent(db_session)
        assert parent == product
        assert product.images[1].file == "test image 2"
        parent = await product.images[1].get_parent(db_session)
        assert parent == product
    ...
    @pytest.mark.asyncio
    async def test_product_deletion_with_images(self, db_session: AsyncSession):
        """Test what happens when trying to delete product with associated images"""
        ...
        # Try to delete product that has associated images
        with pytest.raises(IntegrityError):
            await delete_object(db_session, self.test_product1)
        await db_session.rollback()
    ...
    async def test_orphaned_image_cleanup(self, db_session: AsyncSession):
        """Test handling of images when their product is deleted"""
        ...
        # Create image associated with temp product
        temp_image = Images(
        ...
        # Try to delete the product (should fail due to foreign key)
        with pytest.raises(IntegrityError):
            await delete_object(db_session, temp_product)
        await db_session.rollback()
    ...
    @pytest.mark.asyncio
    async def test_query_product_by_images(self, db_session: AsyncSession):
        """Test querying product by images"""
        # Create images associated with different products
        image1 = Images(
            file="Query Image 1",
            object_id=self.test_product1.id,
            content_type="products"
        ...
        image2 = Images(
            file="Query Image 2",
            object_id=self.test_product2.id,
            content_type="products"
        )
        await save_object(db_session, image2)

        # Query product by images using raw SQL
        stmt = select(Products).join(
            Images,
            and_(
                Products.id == Images.object_id,
                Images.content_type == "products"
            )
        ).where(
