...
class TestGetProducts:
    ...
    async def test_get_products_filter_by_name(
        ...
        response = await async_client.get("/api/v1/products/?name=iPhone")
        assert response.status_code == 200
        data = response.json()["data"]
        assert len(data) == 1
    ...
    async def test_get_products_filter_by_slug(
        ...
        response = await async_client.get("/api/v1/products/?slug=iphone-15")
    ...
    async def test_get_products_filter_by_is_active(
        ...
        response = await async_client.get("/api/v1/products/?is_active=false")
        assert response.status_code == 200
        data = response.json()["data"]
        assert len(data) == 1
...
class TestUpdateProduct:
    ...
    async def test_update_product_success(
        ...
        response = await async_client.put(
            f"/api/v1/products/{product.id}", json=update_data
        )
    ...
    async def test_update_product_not_found(self, async_client: AsyncClient):
        ...
        response = await async_client.put(
            "/api/v1/products/999", json=update_data
        )
    ...
    async def test_update_product_duplicate_name(
        ...
        response = await async_client.put(
            f"/api/v1/products/{product2.id}", json=update_data
        )
...
class TestGetProductSkus:
    ...
    async def test_get_product_skus_not_found(self, async_client: AsyncClient):
        """Test getting SKUs for non-existent product."""
        response = await async_client.get("/api/v1/products/999/skus/")
...
class TestProductEndpointIntegration:
    ...
    async def test_full_crud_workflow(
        ...
        response = await async_client.post("/api/v1/products/", json=create_data)
        assert response.status_code == 201
    ...
    async def test_product_with_skus_workflow(
        ...
        # Get SKUs by product
        response = await async_client.get(f"/api/v1/products/{product.id}/skus/")
        assert response.status_code == 200
        data = response.json()["data"]
        assert len(data) == 2
        names = {item["name"] for item in data}
